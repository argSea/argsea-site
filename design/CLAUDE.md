...
- Hello's layout is the "split watch": letter + photo rack (two operator slots: `watch-postcard`, `watch-postcard-2`), the wave strip doubles as coast wayfinding (beacons anchor to `light-002/004/006` and `chart-log`), and Selected work is the light-list register (lamps blink each light's real characteristic; F W never blinks). The ten-poke cat easter egg phases in `Hello Gazette.dc.html` (The Gull Post).
